...
## Если раньше «пропадали блоки»

Теперь приложение автоматически стабилизирует данные при запуске:
- если список блоков поврежден, пустой или содержит пустые строки, он автоматически восстанавливается в `["Мой блок"]`;
- если у задачи пустой `block` или указан несуществующий блок, задача автоматически привязывается к первому существующему блоку;
- селект блока всегда остается заполненным и с выбранным значением.

То есть состояние «нет блоков / пустой селект / задачи пропали из блока» больше не должно возникать.
